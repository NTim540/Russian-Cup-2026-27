(()=>{
  ...
  const badge=(m)=>m?.fhr_sync_status==='ERROR'?['Ошибка','error']:m?.fhr_sync_status==='SYNCING'?['Синхронизация','syncing']:m?.fhr_match_url?['Источник готов','ready']:['Не привязан',''];
  ...
  function summaryHtml(parsed){
    if(!parsed)return '';
    const score=parsed.headline_score?parsed.headline_score.join(':'):'—';
    const periods=(parsed.period_scores||[]).slice(0,3).map(x=>x.join(':')).join(' · ')||'—';
    const goals=parsed.goals?.length||0;
    return `<div class="fhr-sync-summary"><span class="fhr-chip">Счёт ${esc(score)}</span><span class="fhr-chip">Периоды ${esc(periods)}</span><span class="fhr-chip">Голов ${goals}</span></div>`;
  }

  function panelHtml(m){
    const [label,cls]=badge(m),url=m?.fhr_match_url||'',enabled=Boolean(m?.fhr_sync_enabled),last=m?.fhr_last_checked_at?`Последняя проверка: ${fmt(m.fhr_last_checked_at)}`:'Источник ещё не проверялся',synced=m?.fhr_last_synced_at?` · последняя синхронизация: ${fmt(m.fhr_last_synced_at)}`:'';
    return `<div class="fhr-sync-head"><div class="fhr-sync-title">ФХР LIVE · источник протокола</div><span class="fhr-sync-badge ${cls}">${esc(label)}</span></div>
      ...
      <div class="fhr-sync-actions"><button type="button" class="btn sec small fhr-save">Сохранить привязку</button><button type="button" class="btn sec small fhr-check">Проверить и распознать</button><button type="button" class="btn small fhr-sync-now">Синхронизировать сейчас</button>${url?`<a class="fhr-sync-open" href="${esc(url)}" target="_blank" rel="noopener noreferrer">Открыть протокол ↗</a>`:''}</div>
      <div class="fhr-sync-status">${esc(last+synced)}${m?.fhr_match_id?` · ID ФХР: ${esc(m.fhr_match_id)}`:''}</div>
      <div class="fhr-sync-preview"></div>
      <div class="fhr-sync-note">Сейчас импортируем счёт, счёт по периодам, голы и передачи. События ФХР сохраняются отдельно от ручных и обновляются по внешнему ключу, поэтому повторная синхронизация не создаёт дубли. Удаления добавим следующим этапом.</div>`;
  }
  ...
  function setPreview(box,parsed){const el=box.querySelector('.fhr-sync-preview');if(el)el.innerHTML=summaryHtml(parsed)}
  function setBusy(box,on){box.dataset.busy=on?'1':'0';box.querySelectorAll('button').forEach(b=>b.disabled=on)}
  async function ensureConfigured(box,id){const url=box.querySelector('.fhr-url').value.trim(),enabled=box.querySelector('.fhr-enabled').checked,current=getMatch(id);if(url!==String(current?.fhr_match_url||'')||enabled!==Boolean(current?.fhr_sync_enabled)){const saved=await call({action:'configure',match_id:id,fhr_match_url:url,enabled});mergeMatch(id,saved.match)}return{url,enabled}}
  ...
  function wire(card,box,id){
    ...
    box.querySelector('.fhr-sync-now')?.addEventListener('click',async()=>{
      ...
      try{const cfg=await ensureConfigured(box,id);if(!cfg.enabled)throw Error('Включите «Использовать ФХР как источник»');const b=await call({action:'sync_now',match_id:id});mergeMatch(id,b.match);setPreview(box,b.parsed);const s=b.parsed?.headline_score?b.parsed.headline_score.join(':'):'—',ev=b.events||{};setStatus(box,`Готово: ${s} · событий ${ev.total||0} · новых ${ev.inserted||0} · обновлено ${ev.updated||0}.`,'ok');await loadData(D.tournament.slug,D.stage.id)}catch(e){setStatus(box,e.message||String(e),'bad');setBusy(box,false)}
    });
  }
