(()=>{
  ...
  function writeMap(arenas,map){
    const keep=(Array.isArray(arenas)?arenas:[]).filter(a=>!String(a?.name||'').startsWith(PREFIX));
    const text=JSON.stringify(map||{});if(text==='{}')return keep;
    for(let i=0,n=1;i<text.length;i+=CHUNK,n++)keep.push({name:PREFIX+String(n).padStart(2,'0'),address:text.slice(i,i+CHUNK)});
    return keep;
  }
  async function loadStatic(){
    if(STATIC_ROSTERS)return STATIC_ROSTERS;
    try{const text=await fetch('/rosters.js?v=20260903-1',{cache:'force-cache'}).then(r=>r.text()),a=text.indexOf('const R={'),b=text.indexOf('const NOTES=',a);if(a<0||b<0)throw Error();let expr=text.slice(a+'const R='.length,b).trim();if(expr.endsWith(';'))expr=expr.slice(0,-1);STATIC_ROSTERS=Function('return ('+expr+')')()}catch{STATIC_ROSTERS={}}
    return STATIC_ROSTERS;
  }
  async function roster(team){
    try{const r=await fetch(FHR+'?team='+team.id,{cache:'no-store'});if(r.ok){const b=await r.json();if(Array.isArray(b.players)&&b.players.length)return b.players.map(p=>({number:p.number,name:clean(p.name),pos:p.pos||'U'}))}}catch{}
    const all=await loadStatic(),rows=all?.[team.name]||[];return rows.map(x=>({number:x[0],name:clean(x[1]),pos:x[2]||'U'}));
  }
  ...
  async function saveTeam(team,arenas){
    const body={action:'update_team_profile',id:team.id,city:team.city||'',logo_url:team.logo_url||'',hero_image_url:team.hero_image_url||'',vk_url:team.vk_url||'',telegram_url:team.telegram_url||'',max_url:team.max_url||'',website_url:team.website_url||'',arenas};
    const r=await fetch(TEAM_API,{method:'POST',headers:{'Content-Type':'application/json','x-admin-password':PW},body:JSON.stringify(body)}),b=await r.json().catch(()=>({}));if(!r.ok)throw Error(b.error||'Не удалось сохранить состав');return b;
  }
  async function saveLineup(sec,m,home,away){
    ...
    try{
      const hm=readMap(home.arenas),am=readMap(away.arenas),hsel=selectedNames(sec,'home'),asel=selectedNames(sec,'away');
      ...
      await Promise.all([saveTeam(home,writeMap(home.arenas,hm)),saveTeam(away,writeMap(away.arenas,am))]);
      status.textContent=`Сохранено: ${hsel.length} + ${asel.length} игроков`;
