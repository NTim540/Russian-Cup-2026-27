(()=>{
  ...
  style.textContent=`
    .team-admin-layout{display:grid;grid-template-columns:280px minmax(0,1fr);gap:14px}.team-admin-list,.team-admin-editor{padding:16px}.team-admin-picker{display:grid;gap:7px}.team-admin-preview{display:grid;place-items:center;min-height:190px;margin-top:12px;border:1px solid var(--line);border-radius:14px;background:rgba(255,255,255,.025);overflow:hidden}.team-admin-preview img{max-width:150px;max-height:150px;object-fit:contain;filter:drop-shadow(0 10px 22px rgba(0,0,0,.28))}.team-admin-preview span{color:var(--muted);font-size:12px}.team-admin-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.team-admin-grid .wide{grid-column:1/-1}.arena-list{display:grid;gap:9px;margin-top:10px}.arena-row{display:grid;grid-template-columns:.8fr 1.4fr auto;gap:8px;align-items:end;padding:10px;border:1px solid var(--line);border-radius:12px;background:rgba(255,255,255,.02)}.arena-remove{min-width:42px;height:42px}.team-admin-actions{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:14px}.team-admin-status{color:var(--muted);font-size:11px}.team-admin-note{margin-top:10px;padding:10px 12px;border:1px dashed var(--line);border-radius:11px;color:var(--muted);font-size:11px;line-height:1.5}
  ...
  section.innerHTML=`
    <div class="panel-title"><div><h2 style="margin:0">Команды</h2><div class="muted">Логотип, город, арены, соцсети и официальный сайт.</div></div></div>
    <div class="team-admin-layout">
      <section class="card team-admin-list">
        <div class="team-admin-picker"><div class="field"><label>Команда</label><select id="teamAdminSelect" class="select"></select></div></div>
  ...
  const sel=section.querySelector('#teamAdminSelect'),preview=section.querySelector('#teamAdminPreview'),form=section.querySelector('#teamAdminForm');
  const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  let selectedId=null;

  function teams(){try{return [...(D?.teams||[])].sort((a,b)=>a.name.localeCompare(b.name,'ru'))}catch{return[]}}
  function team(){return teams().find(t=>Number(t.id)===Number(selectedId))||teams()[0]||null}
  ...
  function renderPicker(){const list=teams();const keep=list.some(t=>Number(t.id)===Number(selectedId))?selectedId:list[0]?.id;selectedId=keep||null;sel.innerHTML=list.map(t=>`<option value="${t.id}">${esc(t.name)}</option>`).join('');if(selectedId)sel.value=String(selectedId);renderForm()}

  async function save(){const t=team();if(!t)return;const saveBtn=form.querySelector('#saveTeamProfile'),status=form.querySelector('#teamAdminStatus');const arenas=[...form.querySelectorAll('.arena-row')].map(r=>({name:r.querySelector('.arena-name').value.trim(),address:r.querySelector('.arena-address').value.trim()})).filter(x=>x.name||x.address);const body={action:'update_team_profile',id:t.id,city:form.querySelector('#teamCity').value.trim(),logo_url:form.querySelector('#teamLogoUrl').value.trim(),vk_url:form.querySelector('#teamVk').value.trim(),telegram_url:form.querySelector('#teamTg').value.trim(),max_url:form.querySelector('#teamMax').value.trim(),website_url:form.querySelector('#teamWeb').value.trim(),arenas};saveBtn.disabled=true;status.textContent='Сохраняю…';try{const r=await fetch(API,{method:'POST',headers:{'Content-Type':'application/json','x-admin-password':PW},body:JSON.stringify(body)});const b=await r.json().catch(()=>({}));if(!r.ok)throw Error(b.error||'Не удалось сохранить');status.textContent='Сохранено';await loadData(D.tournament.slug,D.stage.id);selectedId=t.id;renderPicker()}catch(e){status.textContent='Ошибка: '+(e.message||String(e))}finally{saveBtn.disabled=false}}

  sel.addEventListener('change',()=>{selectedId=Number(sel.value);renderForm()});
  btn.addEventListener('click',()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===btn));document.querySelectorAll('[id^="tab-"]').forEach(x=>x.classList.toggle('hidden',x!==section));section.classList.remove('hidden');renderPicker()});
  document.querySelectorAll('.tab:not([data-tab="teams"])').forEach(x=>x.addEventListener('click',()=>section.classList.add('hidden')));
  document.querySelector('#tournament')?.addEventListener('change',()=>setTimeout(()=>{if(!section.classList.contains('hidden'))renderPicker()},350));
  document.querySelector('#stage')?.addEventListener('change',()=>setTimeout(()=>{if(!section.classList.contains('hidden'))renderPicker()},350));
})();