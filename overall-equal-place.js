(()=>{
  ...
  function isOpeningTieFromDom(table){
    ...
    if(!rows.length||gameIndex<0)return false;
    return rows.every(tr=>{
      const cells=[...tr.children];
      return cells[gameIndex]&&num(cells[gameIndex].textContent)===0;
    });
  }
  ...
  function isOpeningTieFromData(){
    try{
      ...
      return rows.length>0&&rows.every(r=>Number(r.gp)===0);
    }catch{return false}
  }
  ...
  function apply(){
    ...
    if(!table)return;
    if(!(isOpeningTieFromDom(table)||isOpeningTieFromData()))return;
